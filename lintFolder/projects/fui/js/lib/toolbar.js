...
(function($, idList, slideClass){
	...
	function ToolBar (opts) {
		...
		var DEFAULT = {
			...
			menuEvent: 'mouseenter', //用来触发侧边栏的类名上的事件
    ...
    ToolBar.prototype.initEvent = function() {
        ...
        //只显示当前面板的后面一个
        _this.$barItem.on('mouseenter', function() {
            var $this = $(this),
                $next = $this.next(),
        ...
        var delayActiveItem = function($item) {
            _this.calcItemLoc($item);
            moveTimer = setTimeout(function() {
                var moveCurrentLoc = _this.status.mouseLocs[_this.status.mouseLocs.length - 1];
                if(_this.adjustInItem(moveCurrentLoc)) {
        ...
        _this.$leftBar.on('mouseenter', '.' + _this.config.item, function(e) {
            ...
            if (_this.status.isOut) {
                ...
                var haschild = $item.data( _this.config.hasChild ),
                    preHaschild =  $item.prev().data( _this.config.hasChild ) || false,
                    nextHasChild = $item.next().data( _this.config.hasChild ) || false;
                ...
                if(haschild) {
                    ...
                    if(preHaschild || nextHasChild) {
                        
                        //如果没有子列表，则判断是否停留200ms，停留则切换
                        delayActiveItem($item);
                    } else {
            ...
            _this.deActiveDi($barItem,function() {
                $('.' + _this.config.item).removeClass(_this.config.active);
            });
            _this.status.isLeftBarOut = false;
            _this.status.mouseLocs = [];

        }).off('mouseleave', '.' + _this.config.item).on('mouseleave', '.' + _this.config.item, function(e) {
    ...
    ToolBar.prototype.calcNextLeftLoc = function($next) {
        var nextOffset = $next.offset(),
            nextTop = nextOffset.top,
            nextLeft = nextOffset.left;
        return {
            upperLeft: {
                x: nextLeft,
                y: nextTop
    ...
    ToolBar.prototype.calcItemLoc = function($item) {
        var _liWidth = $item.outerWidth(),
            ...
            _liLeft = _liOffset.left,
            _liTop = _liOffset.top;
