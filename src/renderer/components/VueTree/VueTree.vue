...
export default {
  ...
  methods: {
    ...
    getNode(path, nodeModel = null, siblings = null) {
      ...
      const node = {
        ...
        children: nodeModel.children
          ? this.getNodes(nodeModel.children, path)
          : [],
        isExpanded:
    ...
    select(clickedNode, event = null, addToSelection = false) {
      addToSelection =
        ((event && event.ctrlKey) || addToSelection) && this.allowMultiselect;
      const newNodes = this.copy(this.value);
      const shiftSelectionMode =
        this.allowMultiselect && event.shiftKey && this.lastSelectedNode;
      const selectedNodes = [];
      ...
      this.traverse((node, nodeModel) => {
        if (shiftSelectionMode) {
          if (
            node.pathStr === clickedNode.pathStr ||
    ...
    onNodeMousemoveHandler(event) {
      ...
      const isDragStarted =
        initialDraggingState === false && this.isDragging === true;
    ...
    onNodeMouseupHandler(event, targetNode = null) {
      ...
      if (this.cursorPosition.placement === "inside") {
        ...
        const insertInd =
          this.cursorPosition.placement === "before"
            ? destNode.ind
            : destNode.ind + 1;
